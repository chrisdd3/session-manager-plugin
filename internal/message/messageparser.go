...
func getUuid(log log.T, byteArray []byte, offset int) (result uuid.UUID, err error) {
	...
	if offset > byteArrayLength-1 || offset+16-1 > byteArrayLength-1 || offset < 0 {
		...
		return result, errors.New("Offset is outside the byte array.")
	}

	leastSignificantLong, err := getLong(log, byteArray, offset)
	if err != nil {
		log.Error("getUuid failed: failed to get uuid LSBs Long value.")
		return result, errors.New("Failed to get uuid LSBs long value.")
	}
...
func putUuid(log log.T, byteArray []byte, offset int, input *uuid.UUID) (err error) {
	...
	uuidBuf, err := input.MarshalBinary()
	if err != nil {
		log.Error("putUuid failed: uuid.MarshalBinary")
		return errors.New("putUuid failed: uuid.MarshalBinary")
	...
	leastSignificantLong, err := bytesToLong(log, uuidBuf[8:16])
	if err != nil {
	...
	mostSignificantLong, err := bytesToLong(log, uuidBuf[0:8])
	if err != nil {
